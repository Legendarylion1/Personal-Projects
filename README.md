...
### Blocking Functions
Blocking functions have become a recent pain point for me. I recently added animations to my game, and some animations can be canceled with a button click handled in the input class. This means that both functions need to be checked at the same time. I could remedy this by having both functions be non-blocking in a loop or thread both functions. In another project, I will thread the renderer, but in this project, I decided to make the functions non-blocking so that both can render and check for input. I wish I thought of this as a design point when I was first creating the game because it would make my code a lot simpler to have an API call to the renderer. Then it would run an animation in another thread using the renderer while I check for input in the main thread. Small things like this are bound to pop up as I increase in scope and complexity, and it helps me know what to consider when I am programming in the future. Another notable point where I "coded myself into a corner" was with the battle simulator. I originally coded the battle simulator to be text-based and then added the renderer to it later. However, I did not yet conceive of the menu class that I would later use in the battle simulator. I refactored the BattleSimulator Class now in the location "src/Battle/OldBattleSimulator.h". Before, I had to create special code in the menu to account for how the battleSimulator functioned but now that I have refactored it, the menu is becoming as generic as possible. The new Battle Simulator is in the location "src/Battle/Battle.h".


### Code Organization
Code organization for this project is going well. I have a lot of classes that all follow the single responsibility principle for design. Code organization was something that I tried to pay a lot of attention to when developing this project. The only class that didn't directly adhere to this structure is the battle simulator. I should have taken more time to design the battle simulator so that I could appropriately break up its code. I have now refactored the BattleSimulator into the new Battle Class, but the idea remains the same. The idea for the battle simulator is that it works in three stages. Stage one is to get the user and AI input. Stage two is to generate all of the battle data for the simulation. Stage three simulates the battle for the user to see. The new Battle Class solved the problems I had with the old BattleSimulator. The main problem was that the code wasn't broken up into individual classes making the old battle simulator two thousand lines long and a pain to debug. The code was so tightly coupled that making changes or adding new functionality was nearly impossible. By breaking up the battle simulator into each of its components and further subdividing those components I was able to create cleaner code that is more maintainable for the future. The more I develop the more I realize the importance of code organization and the need to properly plan out my projects when developing.
