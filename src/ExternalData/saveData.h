...
struct HeaderSave
{
	...
	{
		return playerName + comma + playTime + comma + std::to_string(pokedexFound) + comma + std::to_string(badgesObtained) + "\n";
	}
};
...
struct TrainerSave
{
	...
	operator std::string const()
	{
		return name + comma + std::to_string(money) + comma + std::to_string(x) + comma + std::to_string(y) + comma + std::to_string(direction) + comma + std::to_string(badgeCount) + "\n";
	}
};
