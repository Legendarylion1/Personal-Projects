...
void BattleDataGenerator::setup(Trainer* trainer, SimulationCommand* playerCommand, SimulationCommand* aiCommand, int* playerPokemonIndex, int* opponentPokemonIndex, int* endCondition)
{
	m_trainer = trainer;
...
void BattleDataGenerator::generateStatusData(SelectedUser user, IncrementTime afflictionTime)
{
    //Major Status
    if (user == SelectedUser::PLAYER)
    {
        if (m_playerPokemon.getMajorStatus()->id != StatusID::NO_STATUS && getStatusFromID(m_playerPokemon.getMajorStatus()->id)->getData().time == afflictionTime)
        {
            ...
            if (m_playerPokemon.getMajorStatus()->id != StatusID::NO_STATUS && !m_playerPokemon.getMajorStatus()->isBlocker)
                appendCommands(getStatusFromID(m_playerPokemon.getMajorStatus()->id)->getStatusData(&m_playerPokemon, SelectedUser::PLAYER));
        }
        ...
        for (int i = m_playerPokemon.getMinorStatus()->size() - 1; i >= 0; i--)
        {
            ...
            appendCommands(getStatusFromID(m_playerPokemon.getMinorStatus()->at(i).id)->getStatusData(&m_playerPokemon, SelectedUser::PLAYER));
        }
    }
    else
    {
        if (m_opponentPokemon.getMajorStatus()->id != StatusID::NO_STATUS && getStatusFromID(m_opponentPokemon.getMajorStatus()->id)->getData().time == afflictionTime)
        {
            ...
            if (m_opponentPokemon.getMajorStatus()->id != StatusID::NO_STATUS && !m_opponentPokemon.getMajorStatus()->isBlocker)
                appendCommands(getStatusFromID(m_opponentPokemon.getMajorStatus()->id)->getStatusData(&m_opponentPokemon, SelectedUser::AI));
        }
        ...
        for (int i = m_opponentPokemon.getMinorStatus()->size() - 1; i >= 0; i--)
        {
            ...
            appendCommands(getStatusFromID(m_opponentPokemon.getMinorStatus()->at(i).id)->getStatusData(&m_opponentPokemon, SelectedUser::AI));
        }
    }
}
