...
void BattleMenuGraphics::setup(Trainer* trainer, Menu* menu, int* playerPokemonIndex, int* opponentPokemonIndex, RosterDisplayData* rosterData)
{
	...
	m_menu->setupBattleData(&m_playerMoves, nullptr, m_rosterData, &m_playerBattleItems, &m_battleData, &m_playerPokemonData, &m_opponentPokemonData, &m_learnMoveData);
}
...
void BattleMenuGraphics::updatePokemonDisplay(SelectedUser user)
{
    if (user == SelectedUser::PLAYER)
    {
        ...
        m_battleData.botLine = playerPokemon.getName() + " do"; //TODO: Figure out what this is for
