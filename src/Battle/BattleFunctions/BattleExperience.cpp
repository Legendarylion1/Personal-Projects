...
void BattleFaintHandler::setup(Trainer* trainer, Menu* menu, Animator* animator, BattleMenuGraphics* battleMenuGraphics, Timer* timer, Input* input, DialogueRenderer* dialogueRenderer, int* playerPokemonIndex, int* opponentPokemonIndex)
{
	m_trainer = trainer;
	m_menu = menu;
	m_animator = animator;
...
void BattleFaintHandler::levelPokemon(int index)
{
	...
	SharedString pokemonStatDifference = SharedString{ std::to_string(statIncrease.hp), std::to_string(statIncrease.attack), std::to_string(statIncrease.defense), std::to_string(statIncrease.spAttack),
...
int BattleFaintHandler::getPlayerPokemonSwap(bool canLeave)
{
	...
	while (!(0 <= playerSelection && playerSelection <= 5) || !(m_trainer->getRoster()[playerSelection].getCurHealth() > 0))
	{
