...
void BattleSimulator::handleFaintSelection(SelectedUser swapper)
{
    ...
    if (swapper == SelectedUser::PLAYER)
    {
        ...
        m_menu->clearScenes();
        m_menu->preRun();
        while (m_playerDisplayPokemon[pokemonIndex].getCurHealth() == 0)
        {
            onUpdate();
            m_menu->run();
        }
    ...
    else if (swapper == SelectedUser::AI)
    {
        ...
        if (playerSelection == 1)
        {
            ...
            while ( !(0 <= playerSelection && playerSelection <= 5) || !(m_playerDisplayPokemon[playerSelection].getCurHealth() > 0))
            {
                onUpdate();
                m_menu->run();
...
void BattleSimulator::simulatePokemon(SimulationCommand command)
{
    if (command.commander == SelectedUser::PLAYER)
    {
        ...
        m_menu->resetOverlay(MenuOverlay::ATTACK_SELECT);
        m_menu->resetScene(MenuScene::BATTLE);
        updatePlayerAttacks();
        addToXpRecord();
...
void BattleSimulator::grantXP()
{
    ...
    for (size_t i = 0; i < m_xpRecords[m_opponentPokemonIndex].size(); i++)
    {
        ...
        for (int x = 0; x < xpYield; x++)
        {
            ...
            dripCount = 0;
            while (dripCount < dripCountTarget)
            {
                m_menu->renderMenu();
                m_animator->updateRenderer();
