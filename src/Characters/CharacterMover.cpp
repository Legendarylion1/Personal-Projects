...
{
	if (abs(character->position.coords.screenY) + (m_tileSize * m_timer->getDelta() * m_movementSpeed) >= m_tileSize)
	{
	...
	character->position.coords.screenY += (m_tileSize * m_timer->getDelta() * m_movementSpeed);
...
{
	if (abs(character->position.coords.screenX) + (m_tileSize * m_timer->getDelta() * m_movementSpeed) >= m_tileSize)
	{
	...
	character->position.coords.screenX -= (m_tileSize * m_timer->getDelta() * m_movementSpeed);
	animateMove(character);
...
{
	if (abs(character->position.turnCount) + (m_tileSize * m_timer->getDelta() * m_turnSpeed) >= m_tileSize)
	{
		character->position.turnCount = 0.0f;
		return true;
	}

	character->position.turnCount -= (m_tileSize * m_timer->getDelta() * m_turnSpeed);
